...
main(int argc, char *argv[])
{
    ...
    // Silly way to sort a vector.
    // Assume that the initial portion of the vector has already been sorted, up to the iterator iter.
    ...
    for (auto iter { data.begin() }, end { data.end() }; iter != end;) {
        // Find where *iter belongs by calling the standard algorithm lower_bound,
        // which performs a binary search and returns an iterator that points into data at a position where the value should be inserted.
        int value { *iter };
        auto here { std::lower_bound(data.begin(), iter, value) };
        if (iter == here) {
            ++iter; // already in sorted position
        } else { // re-insert the value at the correct position.
            iter = data.erase(iter);
            data.insert(here, value);
        }
    }

    // Debugging code: check that the vector is actually sorted.
    // Do this by comparing each element with the preceding element in the vector.
    ...
    // Print the sorted vector all on one line.
