...
template <class T = long, int N = 2> class currency {
public:
    ...
    template <class U, int M> constexpr currency &operator=(currency<U, M> rhs)
    {
        value_ = rhs.value();
        locale_ = rhs.locale();
        return *this;
    ...
    template <class Char, class Traits> void read(std::basic_istream<Char, Traits> &strm);
    ...
    {
        return value().template convert<U>();
    }
...
template <class T, int N>
constexpr currency<T, N> &
currency<T, N>::operator+=(currency f)
{
    value_ += f.value();
    return *this;
}
...
template <class T, int N>
constexpr currency<T, N> &
currency<T, N>::operator*=(value_type i)
{
    value_ *= i;
    return *this;
}
template <class T, int N>
constexpr currency<T, N> &
currency<T, N>::operator*=(int_type i)
{
    value_ *= i;
    return *this;
}
...
void
currency<T, N>::read(std::basic_istream<CharT, Traits> &strm)
{
...
template <class T, int N>
constexpr currency<T, N>
operator+(currency<T, N> a, currency<T, N> b)
{
    a += b;
    return a;
}
...
operator/(currency<T, N> n, currency<T, N> d)
{
