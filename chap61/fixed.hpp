...
public:
    ...
    static value_type constexpr places10 { ipower(10, N) }; ///< 10<sup>places</sup>
    ...
    constexpr fixed(value_type integer, value_type fraction);
    ...
    template <class Char, class Traits> bool read(std::basic_istream<Char, Traits> &strm);
    /// Convert to long double.
    constexpr double as_long_double() const { return static_cast<long double>(value()) / places10; }
    /// Convert to double.
    constexpr double as_double() const { return static_cast<double>(value()) / places10; }
    /// Convert to float
    constexpr float as_float() const { return static_cast<float>(value()) / places10; }
    ...
    constexpr value_type round() const;

    /// Return the integer part (which is the same as trunc()).
    constexpr value_type integer() const { return value() / places10; }
    /// Return the fractional part, e.g., 3 for 12.03
    constexpr value_type fraction() const;
...
private:
    ...
    constexpr value_type reduce(value_type frac);
    value_type value_;
};

// Construct a fixed value from an integer part and a fraction part
template <class T, int N> constexpr fixed<T, N>::fixed(value_type integer, value_type fraction)
{
    if (fraction < T()) {
...
constexpr fixed<T, N>::fixed(value_type integer)
...
template <class T, int N>
constexpr typename fixed<T, N>::value_type
fixed<T, N>::fraction() const
{
    // std::abs() is not constexpr so don't call it.
    if (value() < 0) {
...
template <class T, int N>
constexpr typename fixed<T, N>::value_type
fixed<T, N>::reduce(value_type frac)
{
...
bool
fixed<T, N>::read(std::basic_istream<Char, Traits> &strm)
{
    ioflags flags { strm };
    ...
    if (not(strm >> decimal) or decimal != '.') {
        ...
        for (; p != places + 1 and strm >> c and std::isdigit(c, strm.getloc()); ++p) {
...
template <class T, int N>
constexpr fixed<T, N>
operator+(fixed<T, N> a, fixed<T, N> b)
{
    a += b;
    return a;
}
