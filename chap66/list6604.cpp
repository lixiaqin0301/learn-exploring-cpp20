...
void
print_file_type(std::ostream &stream, fsys::directory_entry const &entry)
{
...
uintmax_t
get_file_size(fsys::directory_entry const &entry)
{
    std::error_code ec;
...
fsys::file_time_type
get_last_write_time(fsys::directory_entry const &entry)
{
    std::error_code ec;
...
void
print_file_info(std::ostream &stream, fsys::directory_entry const &entry)
{
    std::format_to(std::ostreambuf_iterator<char>(stream), "{0:>16} {1:%F %T} {2}", get_file_size(entry), std::chrono::time_point_cast<std::chrono::seconds>(std::chrono::clock_cast<std::chrono::system_clock>(get_last_write_time(entry))), entry.path().generic_string());
