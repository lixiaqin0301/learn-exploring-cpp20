...
void
print_file_type(std::ostream &stream, fsys::path const &path)
{
...
void
print_file_info(std::ostream &stream, fsys::path const &path)
{
    std::format_to(std::ostreambuf_iterator<char>(stream), "{0:>16} {1:%F %T} ", fsys::file_size(path), std::chrono::time_point_cast<std::chrono::seconds>(std::chrono::clock_cast<std::chrono::system_clock>(fsys::last_write_time(path))));
