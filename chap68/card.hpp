...
std::ostream &
operator<<(std::ostream &stream, card c)
{
    std::string text;
...
std::istream &
operator>>(std::istream &stream, card &c)
{
...
bool
acehigh_less(card a, card b)
{
    return a.get_rank() < b.get_rank() or (a.get_rank() == b.get_rank() and a.get_suit() < b.get_suit());
}

bool
acelow_less(card a, card b)
{
    return (static_cast<int>(a.get_rank()) % 13) < (static_cast<int>(b.get_rank()) % 13) or (a.get_rank() == b.get_rank() and a.get_suit() < b.get_suit());
}
