...
template <class T> class derived : public inner::base {
public:
...
template <class T> typename derived<T>::value_type derived<T>::example = 2;
}

template <class T> class more_derived : public outer::derived<T> {
public:
    typedef outer::derived<T> base_type;
    typedef typename base_type::value_type value_type;
    more_derived(value_type v)
        : value_ { this->value(v) }
    {
    }
