 ...
 * This program is Project 1 in <em>Exploring C++ 20</em>, by Ray Lischner (Apress).
 *
 * Your task is to write a program that reads records, prints the records, and computes some statistics.
 ...
 * weight in kilograms, height in centimeters (not meters), and the person's sex ('M' or 'F').
 * Let the user enter the sex in uppercase or lowercase.
 * Ask the user to enter the height in centimeters, so you can compute the BMI using integers.
 ...
 * Use an asterisk after the BMI rating to mark records for which the number meets or exceeds the threshold.
 * After each table, print the mean (average) and median BMI.
 * (Median is the value at which half the BMI values are less than the median and half are greater than the median.
 * If the user enters an even number of records, take the mean of the two values in the middle.)
 * Compute individual BMI values as integers.
 * Compute the mean and median BMI values as floating-point numbers, and print the mean with one place after the decimal point.
 *
 * BMI is defined as weight in kg/(height in m)<sup>2</sup>, converted to a unitless number.
 *
 * Source file:
 * @author Ray Lischner
 */
 ...
 * The height is in centimeters, so the computed value is off by 10,000.
 * Integer division truncates, but we want to round off to nearest, so add 0.5 and then truncate by casting to an integer.
 *
...
get_record(std::vector<std::string> &names, std::vector<int> &heights, std::vector<int> &weights, std::vector<char> &sexes)
{
    ...
    // Enforce minimal sanity check on the height, which must be between 10 and 300 cm, or baby- to giant-size.
    ...
    // Enforce minimal sanity check on the weight, which must be between premature-baby and giant size.
    ...
    // All information has now been collected, so append it all to the respective vectors.
...
print_table(char sex, std::vector<int> const &heights, std::vector<int> const &weights, std::vector<int> const &bmis, std::vector<char> const &sexes, std::vector<std::string> const &names, int threshold)
{
    ...
    std::vector<int> tmpbmis; // store only the BMIs that are printed to compute the median
    ...
    if (bmi_count != 0) {
        ...
        // Median BMI is trickier. The easy way is to sort the array and pick out the middle item or items.
        // There is also a standard algorithm that lets you determine the median in linear time, but the book has not discussed it.
