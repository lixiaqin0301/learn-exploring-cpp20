...
template <class C> struct text : std::basic_string<C> {
    ...
    constexpr explicit operator bool() const noexcept { return not this->empty(); }
};
...
template <class Ch>
auto
read(std::basic_istream<Ch> &in) -> std::vector<text<Ch>>
{
