...
public:
    ...
    enum kind : int { eof = 0, identifier = 'A', integer = 'I', floating_point = 'F', string = 'S', plus = '+', minus = '-', times = '*', slash = '/', lparen = '(', rparen = ')', equal = '=', comma = ',' };
...
private:
     ...
     */
    template <class OutputIterator> OutputIterator get_namelist(OutputIterator output);
...
OutputIterator
parser::get_namelist(OutputIterator output)
{
    std::string token {};
