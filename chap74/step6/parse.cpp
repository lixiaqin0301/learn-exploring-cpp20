...
void
parser::get_escape(std::string &str)
{
    char c;
    if (not input_.get(c))
...
void
parser::get_string(std::string &result, char delimiter)
{
    char c {};
    while (input_.get(c)) {
        if (c == delimiter)
...
void
parser::get_identifier(std::string &identifier)
{
    identifier.clear();
    char c {};
    if (not input_.get(c))
...
parser::kind
parser::get_token(std::string &token)
{
    if (not token_.empty()) {
        kind result(kind_);
        token = token_;
...
bool
parser::get_integer(std::string const &token, node &result)
{
...
bool
parser::get_float(std::string const &token, node &result)
{
    std::istringstream stream { token };
    // If the value overflows or is otherwise invalid, return false.
    double value {};
...
void
parser::get_definition(std::string &name, identifier_list &parameters, node &definition)
{
    // Define a variable.
...
bool
parser::get_statement(std::ostream &output)
{
    std::string token {};
...
void
parser::get_expr_list(node_list &result)
{
    result.clear();
    std::string token {};
    while (kind k = get_token(token)) {
...
bool
parser::get_primary(node &result)
{
    std::string token {};
    kind k = get_token(token);
    if (k == eof)
...
void
parse_loop(std::istream &input, std::ostream &output)
{
    std::string line {};
